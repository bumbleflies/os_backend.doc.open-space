...
async def update_session(new_session: TransientSessionData, session=Permission('update', get_session),
                         response: Response = None) -> SessionData | ErrorMessage:
    if session_registry.has_session(session.os_identifier, session.identifier):
        return session_registry.update_session(session.os_identifier, session.identifier, new_session)
    else:
        response.status_code = status.HTTP_404_NOT_FOUND
        return ErrorMessage('Invalid OpenSpace Identifier')
...
async def delete_session(session=Permission('delete', get_session)) -> None:
